// generated by VivioJS 22.02 build 0 : 14-Feb-22 14:27:58

"use strict"

function bubbleSort(vplayer) {
	...
	function Limit($grp, x, y) {
		...
		new Line2(this, 0, 0, $g[33], 0, 0, 0, 270)
		this.$rightArrowPen=new SolidPen(0, 6, BLACK, ARROW60_END)
		new Line2(this, 0, 0, this.$rightArrowPen, 10, 5, 40, 0)
		new Txt(this, 0, HLEFT|VTOP, 10, 15, $g[1], $g[36], "sorted")
	}
	...
	function $execute(thread) {
		...
		while (1) {
			...
			case 21:
				...
				$g[55]=new Rectangle2($g[0], 0, HCENTRE, 0, $g[35], BORDER, TITLEY, W/2, TITLEH, $g[2], $g[40], "Bubble Sort")
				$g[55].setRounded(5, 5)
				$g[55].setTxtOff(0, 2)
			...
			case 23:
				...
				continue
			case 24:
				$g[57]=new Limit($g[0], $g[56]+N*(SZ+GAP)-GAP/2, Y-130)
				$g[57].setOpacity(0)
			...
			case 37:
				$enter(0);	// flash
				$g[59].setPos($g[56]+SZ/2+$stack[$fp-3]*(SZ+GAP), Y+SZ, TICKS, 0)
				$pc = 38
				if ($g[59].setOpacity(1, TICKS, 1)) {
					return
				}
			case 38:
				$g[54][$stack[$fp-3]].toggleBrushAndPens($g[19], $g[3], $g[54][$stack[$fp-3]].getTxtPen(), 5, getTPS()/4, 0)
				$pc = 39
				if ($g[54][$stack[$fp-3]+1].toggleBrushAndPens($g[19], $g[3], $g[54][$stack[$fp-3]+1].getTxtPen(), 5, getTPS()/4, 1)) {
					return
				}
			case 39:
				$return(1)
			...
			case 43:
				...
				$g[54][$stack[$fp-3]+1].setPos($g[56]+SZ/2+($stack[$fp-3]+1)*(SZ+GAP), Y)
				$g[54][$stack[$fp-3]+1].setBrush($g[18])
				$return(1)
				continue
			case 44:
				$enter(0);	// compare
				$g[64].setTxt("Compare adjacent elements and swap if NOT in ascending order")
				$call(37, $obj, $stack[$fp-3])
				continue
			case 45:
				$g[60]++
			...
			case 46:
				...
				$g[53][$stack[$fp-3]]=$g[53][$stack[$fp-3]+1]
				$g[53][$stack[$fp-3]+1]=$stack[$fp+1]
			...
			case 48:
				$enter(0);	// update
				$g[64].setTxt("Move limit to where last swap took place on previous pass")
				$g[59].setOpacity(0, TICKS, 0)
				$pc = 49
				if ($g[57].translate(-(SZ+GAP)*$stack[$fp-3], 0, TICKS, 1)) {
					return
				}
			case 49:
				$g[59].setPos($g[56]+SZ/2, Y+SZ)
				$pc = 50
				if ($g[59].setOpacity(0, TICKS, 1)) {
					return
				}
			case 50:
				$return(1)
			...
			case 54:
				...
				$call(44, $obj, $stack[$fp+3])
				continue
			case 55:
				if (!($g[53][$stack[$fp+3]]>$g[53][$stack[$fp+3]+1])) {
					$pc = 57
					continue
				}
				$call(46, $obj, $stack[$fp+3], $stack[$fp+3]+1)
				continue
			case 56:
			...
			case 57:
				...
				continue
			case 58:
				$call(48, $obj, $stack[$fp+1]-$stack[$fp+2])
				continue
			case 59:
